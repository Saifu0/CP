...
bool bipartite(vector<int>*edges, int n){
	unordered_set<int> sets[2];  //Two sets to store vectices of bipartite
	vector<int> pending; //Queue
	sets[0].insert(0);
	pending.push_back(0);

	while(pending.size() != 0){
		int currrent = pending.back();
		pending.pop_back(); 
		int currentSet = sets[0].count(currrent) > 0 ? 0 : 1;
		for(int i=0;i<edges[currrent].size();i++){
			int neighbor = edges[currrent][i];
